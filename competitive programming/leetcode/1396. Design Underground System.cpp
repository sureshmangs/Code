...
public:
    ...
    void checkOut(int id, string stationName, int checkOutTime) {
        string sourceStation = checkingIn[id].first;
        int checkInTime = checkingIn[id].second;
        checkingIn.erase(id);
        int journeyTime = checkOutTime - checkInTime;
        
        string srcToDst = sourceStation + "-" + stationName;
        
        if (checkingOut.find(srcToDst) == checkingOut.end()) {
            checkingOut[srcToDst] = {journeyTime, 1};
        } else {
            checkingOut[srcToDst].first += journeyTime;
            checkingOut[srcToDst].second += 1;
        }
    }
    
    double getAverageTime(string startStation, string endStation) {
        string srcToDst = startStation + "-" + endStation;
        return double(checkingOut[srcToDst].first) / double(checkingOut[srcToDst].second);
    }
};
